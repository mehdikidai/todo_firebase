import "./style.scss";
import { todoRef, docRefById } from "./firebase.config.js";
import td from "./lib/td.js";
import { z } from "zod";
import {
    addDoc,
    Timestamp,
    onSnapshot,
    deleteDoc,
    updateDoc,
    query,
    orderBy,
    limit,
    getDocs,
} from "firebase/firestore";
import moment from "moment";

const list = document.getElementById("list");
const contentSchema = z.string().min(2).max(40).trim();
const form = document.getElementById("form");
const todo_text = document.getElementById("todo_text");

let page = 6;

window.addEventListener("load", function () {
    this.setTimeout(() => {
        this.document.getElementById("loading_box").remove();
    }, 1000);
});

form.addEventListener("submit", (e) => {
    e.preventDefault();

    if (contentSchema.safeParse(todo_text.value).success) {
        addDoc(todoRef, {
            content: todo_text.value.replace(/["<>/'``]/gi, "_"),
            done: false,
            date: Timestamp.fromDate(new Date()),
        }).then((res) => {
            console.log(res.id);
            form.reset();
        });
    } else {
        swal({
            title: "mkin walo ?",
            text: "Lorem Ipsum is simply dummy",
            icon: "data:image/svg+xml;base64,",
            button: {
                text: "Close",
            },
        });
    }
});

const q = query(todoRef, orderBy("date", "desc"), limit(page));

onSnapshot(q, (snap) => {
    const data = snap.docs.map((el) => ({
        id: el.id,
        content: el.data().content,
        done: el.data().done,
        date: el.data().date.toDate(),
    }));

    Todo.showData(data);

    getDocs(todoRef).then((res) => {

        //console.log(res.size);

        document.getElementById("h1_logo").dataset.count =
            res.size < 10 ? "0" + res.size : "+9";
    });
    
});





const observer = new MutationObserver(() => {
    document
        .querySelectorAll(".delete_todo")
        .forEach((el) =>
            el.addEventListener("click", (t) =>
                Todo.delete(t.target.dataset.id)
            )
        );
    document
        .querySelectorAll(".done_todo")
        .forEach((el) =>
            el.addEventListener("click", (t) => Todo.done(t.target.dataset))
        );

    document
        .querySelectorAll(".update_todo_btn")
        .forEach((el) =>
            el.addEventListener("click", (t) =>
                Todo.update(t.target.dataset)
            )
        );
    document
        .querySelectorAll(".show_todo")
        .forEach((el) =>
            el.addEventListener("click", (t) =>
                Todo.showInfo(t.target.dataset)
            )
        );
});

observer.observe(list, { childList: true });

setInterval(() => {
    document.getElementById("date_new").innerHTML = moment().format("MMMM Do YYYY, h:mm:ss a");
}, 1000);



class Todo {
    static update = ({ id, content }) => {
        const docRef = docRefById(id);
        //console.log(id, content);
        swal({
            title: "Are you sure?",
            //text: 'Search for a movie. e.g. "La La Land".',
            content: {
                element: "input",
                attributes: {
                    value: content,
                },
            },
            buttons: ["Cancel", "Update"],
        }).then((txt) => {
            console.log(txt);
            if (contentSchema.safeParse(txt).success) {
                try {
                    updateDoc(docRef, { content: txt, done: false });
                } catch (err) {
                    console.error(err);
                }
            }

            swal.close();
        });
    };

    static done = ({ id, done }) => {
        const isTrue = /^true$/i.test(done);
        const docRef = docRefById(id);

        try {
            updateDoc(docRef, { done: !isTrue });
        } catch (err) {
            //console.error(err);
        }
    };

    static delete = (id) => {
        const docRef = docRefById(id);

        swal({
            title: "Are you sure?",
            text: "Lorem Ipsum is simply dummy text of the",
            icon: "data:image/svg+xml;base64,",
            buttons: ["Cancel", "Delete"],
            dangerMode: true,
            className: "ttt",
        }).then((willDelete) => {
            if (willDelete) {
                deleteDoc(docRef).then((res) => {
                    //console.log(res);
                });
            }
        });
    };

    static showInfo = (todo) => {
        swal({
            title: "Created At",
            text: moment(new Date(todo.date)).format("LLL"),
            button: "close",
        });
    };

    static showData = (arr) => {
        list.innerHTML = "";
        if (arr.length > 0) {
            arr.forEach((el) => {
                list.innerHTML += td(el);
            });
        } else {
            list.innerHTML = `<div class='empty'><span>Empty</span><i class='material-symbols-outlined'>scan_delete</i></div>`;
        }
    }
}
